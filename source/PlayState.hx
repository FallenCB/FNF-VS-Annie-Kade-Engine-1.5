	...
	override public function create()
	{
		...
		switch(stageCheck)
		{
			...
			case 'city': 
					...
					var bg:FlxSprite = new FlxSprite(-100).loadGraphic(Paths.image('city/sky'));
					bg.scrollFactor.set(0.1, 0.1);
					add(bg);

					var city:FlxSprite = new FlxSprite(-10).loadGraphic(Paths.image('city/city'));
					city.scrollFactor.set(0.3, 0.3);
					city.setGraphicSize(Std.int(city.width * 0.85));
					city.updateHitbox();
					add(city);
					...
					for (i in 0...5)
					{
							var light:FlxSprite = new FlxSprite(city.x).loadGraphic(Paths.image('city/win'));
							light.scrollFactor.set(0.3, 0.3);
							light.visible = false;
							light.setGraphicSize(Std.int(light.width * 0.85));
							light.updateHitbox();
					...
					var streetBehind:FlxSprite = new FlxSprite(-40, 50).loadGraphic(Paths.image('city/behindTrain'));
					add(streetBehind);

					phillyTrain = new FlxSprite(2000, 360).loadGraphic(Paths.image('city/train'));
					if(FlxG.save.data.distractions){
						add(phillyTrain);
					}
					...
					var street:FlxSprite = new FlxSprite(-40, streetBehind.y).loadGraphic(Paths.image('city/street'));
					add(street);
			...
			{
					curStage = 'cave';
					var bg:FlxSprite = new FlxSprite(-400, -500).loadGraphic(Paths.image('cave/evilBG'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.2, 0.2);
					bg.active = false;
					bg.setGraphicSize(Std.int(bg.width * 0.8));
					...
					var evilTree:FlxSprite = new FlxSprite(300, -300).loadGraphic(Paths.image('cave/evilTree'));
					evilTree.antialiasing = true;
					evilTree.scrollFactor.set(0.2, 0.2);
					add(evilTree);

					var evilSnow:FlxSprite = new FlxSprite(-200, 700).loadGraphic(Paths.image("cave/evilSnow"));
						evilSnow.antialiasing = true;
					add(evilSnow);
			}
		}
